...
const getAccurateDuration = (length, duration) =>
  length > duration ? duration : length;
  ...
  parts.forEach(({ name, duration }) => {
    ...
    if (sampleDataMap && sampleDataMap.duration >= duration && length > 0) {
      ...
      if (length > 0) {
        mp3CompilationArray.push({
          name: path.join(CONFIG[FEATURE_NAME.SAMPLE].FOLDER, SILENCE_FILE),
          duration: getAccurateDuration(
            length,
            duration - sampleDataMap.duration
          ),
        });
...
export const mergeTrack = (compilationArray, newName) => {
  return new Promise((resolve, reject) => {
    compilationArray
      .reduce(
        (prev, curr) =>
          prev.input(curr.name).inputOption(`-t ${curr.duration}`),
        ffmpeg()
      )
      ...
      .mergeToFile(CONFIG[FEATURE_NAME.TRACK].FOLDER + `/${newName}.mp3`, {
        end: true,
      });
  });
};
